...
> &#x2705; \\(\varepsilon_\theta \\) 的预测结果应趋近于 \\(\varepsilon\\) 。   
...
> &#x2753; 怎样衡量两个分布是否接近？   
> &#x2705; Pdata 代表真实分布，从分布中 Sample 出来的 \\(x\\) 即训练集 \\(P_\theta (x^i)\\) 代有 \\(P_\theta\\) 生成 \\(x^i\\) 的概率，但由于 \\(P_\theta\\) 非常复杂，算不出这个概率，但此处假设 \\(P_\theta (x^i)\\) 已知。    
