...
1. 首先将这些 3D 高斯投影到基于像素的图像平面上，这一过程称为“splatting”。  
![](./assets/屏幕快照%202024-05-05%2022.42.06.png)  
2. 然后，对这些高斯函数进行排序并计算每个像素的值。  
![](./assets/7b6ce0e0b104d36e8331f86fd4c9ab5a_3_Figure_4_1136390164.png)  
如图所示，NeRF和3D GS的渲染可以看作是彼此的逆过程。  
3. 接下来，定义 3D 高斯，它是 3D GS 中场景表示的最小元素。
4. 接下来，使用这些 3D 高斯函数进行可微分渲染。
5. 最后，介绍3D GS中使用的加速技术，这是快速渲染的关键。

## 如何为给定场景获取构造良好的 3D 高斯，即3D GS的优化过程。
