...
高斯球数以百万计，控制信号（一段文字、一个力向量）无法直接映射到每个球的偏移量。因此几乎所有方法都在解决同一个问题：**建立一个"中间代理"来承接控制信号，再由代理带动高斯球运动**。代理的形态决定了这个方向的技术分支。

> 代理（Agent）的本质是运动简化 — 通过引入一个中间表示，降低"从控制信号直接学习每个高斯球运动"的复杂度。
