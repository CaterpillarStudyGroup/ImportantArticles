...
||触发条件|动作|要求|
|---|---|---|---|
||最高点碰路边缘黄线（有线以线，没线以缝）|往右一圈半（+1.5）|
||雨刷进线|回半圈（+1.0）|
||左侧角进线|回一圈（0.0）|最高点对箭头左下角<br>最高点对『单人旁』<br>最高点对第三根线左侧线头，或第二、三线中间|
||前轮上坡|加油（0.0）|最高点对黑白交接处|
||车头朝下|刹车（0.0）|最高点对黑白交接处|
||后轮着地|停车（0.0），打右转向灯，启动|
||启动|直行（0.0）|左侧角沿左侧黄线右侧边|
||最高点离黄线20-30cm|关转向灯|
||右门把手对准黄线|往右一圈关（+1.5）|
||等车直了|回半圈（+1.0），再打回半圈（+1.5）|
||雨刷进黄线|回半圈（+1.0）|
||左侧角进黄线|回一圈（0.0）|
||左侧角盖住绿线外侧边|往左打一圈（-1.0）|左侧角沿着绿线外侧边（超出黄线10cm）走|
||左侧角进线|放平（？）|
||左侧角盖住第二根绿线的外侧边|回一圈（0.0）|
||右侧筋盖住绿线外侧边|往右打一圈（+1.0）|右侧筋沿着绿线外侧边走|
||右侧筋与最高点中间位置碰黄线|回一圈（0.0）|雨刷沿侧方停车的虚线，最高点对准右边白点|
||黄线消失|看右后视镜|
||后视镜中横线到镜子一半高度|停，倒档|
||后视镜中横线消失|往右一圈半（+1.5），同时人往左看|
||小线帽进入左后视镜|回一圈半（0.0），同时人往左探头看|
||左后轮压线|往左一圈半（-1.5）|
||左侧角进黄线|停车，打左转向灯，挂档|
||左侧角盖住左侧黄线|回一圈半（0.0）|
||雨刷不到10cm（？）|打一圈（+1.0）|
